...
#
# NOTE:
# Some environments initialize the SQLite schema from SQL scripts (db/schema.sql).
# If these tables already exist, the migration must not crash. This migration is
# therefore written to be idempotent on SQLite (CREATE TABLE/INDEX IF NOT EXISTS),
...
def _ensure_sqlite_schema(apps, schema_editor):
    ...
    # If tables already exist (created by scripts or previous runs),
    ...
    with schema_editor.connection.cursor() as cursor:
        ...
        # Create indexes only after required columns exist.
