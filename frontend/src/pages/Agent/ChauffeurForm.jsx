import React, { useEffect, useState } from 'react';
import { useNavigate, useParams } from 'react-router-dom';
import api from '../../api';
import PageHeader from '../../components/PageHeader';
import { ArrowLeft } from 'lucide-react';

const ChauffeurForm = () => {
    const { id } = useParams();
    const isEdit = Boolean(id);
    const navigate = useNavigate();

    const [formData, setFormData] = useState({
        nom: '',
        prenom: '',
        telephone: '',
        num_permis: '',
        statut: 'actif'
    });

    const [loading, setLoading] = useState(false);
    const [fetching, setFetching] = useState(isEdit);

    useEffect(() => {
        const fetchChauffeur = async () => {
            if (!isEdit) return;
            try {
                const res = await api.get(`/chauffeurs/${id}/`);
                const c = res.data;
                setFormData({
                    nom: c.nom || '',
                    prenom: c.prenom || '',
                    telephone: c.telephone || '',
                    num_permis: c.num_permis || '',
                    statut: c.statut || 'actif'
                });
            } catch (err) {
                console.error(err);
                alert("Impossible de charger le chauffeur");
                navigate('/chauffeurs');
            } finally {
                setFetching(false);
            }
        };
        fetchChauffeur();
    }, [id, isEdit, navigate]);

    const handleChange = (e) => {
        const { name, value } = e.target;
        setFormData(prev => ({ ...prev, [name]: value }));
    };

    const handleSubmit = async (e) => {
        e.preventDefault();
        setLoading(true);
        try {
            if (isEdit) {
                await api.put(`/chauffeurs/${id}/`, formData);
            } else {
                await api.post('/chauffeurs/', formData);
            }
            navigate('/chauffeurs');
        } catch (err) {
            console.error(err);
            const msg = err.response?.data?.detail || "Erreur lors de l'enregistrement du chauffeur";
            alert(msg);
        } finally {
            setLoading(false);
        }
    };

    if (fetching) return <div className="page-container">Chargement...</div>;

    return (
        <div className="page-container">
            <button
                onClick={() => navigate('/chauffeurs')}
                style={{
                    background: 'transparent',
                    border: 'none',
                    color: 'var(--text-muted)',
                    display: 'flex',
                    alignItems: 'center',
                    gap: '0.5rem',
                    marginBottom: '1rem',
                    cursor: 'pointer',
                    fontSize: '0.875rem',
                    padding: '0.5rem 0'
                }}
            >
                <ArrowLeft size={18} />
                Retour aux chauffeurs
            </button>

            <PageHeader
                title={isEdit ? 'Modifier le chauffeur' : 'Ajouter un chauffeur'}
                subtitle={isEdit ? 'Modifiez les informations du chauffeur' : 'Enregistrez un nouveau conducteur dans la flotte'}
            />

            <div className="form-card" style={{ maxWidth: '800px' }}>
                <form onSubmit={handleSubmit}>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem' }}>
                        <div className="form-group">
                            <label>Nom *</label>
                            <input name="nom" value={formData.nom} onChange={handleChange} placeholder="Nom" required />
                        </div>

                        <div className="form-group">
                            <label>Prénom *</label>
                            <input name="prenom" value={formData.prenom} onChange={handleChange} placeholder="Prénom" required />
                        </div>

                        <div className="form-group">
                            <label>Téléphone</label>
                            <input name="telephone" value={formData.telephone} onChange={handleChange} placeholder="+221 ..." />
                        </div>

                        <div className="form-group">
                            <label>Numéro de Permis</label>
                            <input name="num_permis" value={formData.num_permis} onChange={handleChange} placeholder="N° de permis de conduire" />
                        </div>

                        <div className="form-group">
                            <label>STATUT</label>
                            <select
                                name="statut"
                                value={formData.statut}
                                onChange={handleChange}
                                style={{
                                    borderRadius: '12px',
                                    appearance: 'none',
                                    backgroundImage: `url('data:image/svg+xml;charset=US-ASCII,<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="%23CBD5E0" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M6 9l6 6 6-6"/></svg>')`,
                                    backgroundRepeat: 'no-repeat',
                                    backgroundPosition: 'right 12px center',
                                    paddingRight: '35px',
                                    cursor: 'pointer'
                                }}
                            >
                                <option value="actif">Actif</option>
                                <option value="conge">En congé</option>
                                <option value="indisponible">Indisponible</option>
                            </select>
                        </div>
                    </div>

                    <div style={{ marginTop: '2.5rem', display: 'flex', gap: '1.25rem' }}>
                        <button
                            type="submit"
                            disabled={loading}
                            style={{
                                borderRadius: '25px',
                                padding: '0.75rem 2rem',
                                fontSize: '1rem',
                                fontWeight: '600',
                                background: '#0d9488',
                                color: 'white',
                                border: 'none',
                                cursor: 'pointer',
                                transition: 'all 0.2s'
                            }}
                        >
                            {loading ? 'Enregistrement...' : isEdit ? 'Enregistrer les modifications' : 'Ajouter le chauffeur'}
                        </button>
                        <button
                            type="button"
                            className="secondary"
                            onClick={() => navigate('/chauffeurs')}
                            style={{ borderRadius: '25px', padding: '0.75rem 1.5rem', cursor: 'pointer' }}
                        >
                            Annuler
                        </button>
                    </div>
                </form>
            </div>
        </div>
    );
};

export default ChauffeurForm;
